	...
	public override bool Execute() {
		...
		if (!string.IsNullOrEmpty(BaseManifest) && !"new".Equals(BaseManifest, StringComparison.OrdinalIgnoreCase)) {
			...
			if (!File.Exists(basePath)) {
				...
				try {
					manifest = JsonConvert.DeserializeObject<ModManifest>(File.ReadAllText(basePath)) ?? throw new ArgumentNullException("returned data was null");
		...
		Dictionary<string, VersionBehavior?> checkBehaviors = [];
		...
		if (UpdateKeys != null) {
			var processingKeys = UpdateKeys
				.SelectMany(x => string.IsNullOrEmpty(x) ? [] : x.Split(','))
				.Select(x => x.Trim())
		...
		if (ContentPacks != null) {
			...
			for (int i = 0; i < ContentPacks.Length; i++) {
				...
				try {
					packManifest = JsonConvert.DeserializeObject<ModManifest>(File.ReadAllText(maniPath)) ?? throw new ArgumentNullException("returned data was null");
